...
void print_string_at_vgap(uint16_t x, uint16_t y, const char* str, VGAColor color)
{
    while(*str != '\0')
    {
        print_char_at_vgap(x, y, *str, color);
        x += 8;
...
{    
    ...
    for (uint8_t row = 0; row < 8; row++)
    {
        ...
        for (int8_t col = 0; col < 8; col++)
        {
            ...
            if(bit == 1)
            {
                drawPixel(vgax + offx, vgay, color);
            }
            else
            {
                drawPixel(vgax + offx, vgay, VGA_Color_BLACK);
            }
